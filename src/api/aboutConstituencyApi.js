...
const [newAssemblyFormData, setNewAssemblyFormData] = useState({
  ac_number: '',
  ac_name: '',
  district: '',
  type: ''
...
const saveCurrentAssembly = async () => {
  try {
    // Validate current assembly data
    ...
    // Get current assembly entry
    const currentAssemblyEntry = assemblyConstituencies[currentAssemblyIndex];
    
    // Create updated array with the modified entry
...
const handleAddNewAssembly = async () => {
  ...
  if (!newAssemblyFormData.ac_number || !newAssemblyFormData.ac_name || !newAssemblyFormData.district) {
    Alert.alert('Validation Error', 'Please fill in all required fields (AC Number, AC Name, and District)');
    return;
  }
  ...
  setSaveLoading(true);
  try {
    ConstituencyLoggingService.constInfo('➕ === ADDING NEW ASSEMBLY CONSTITUENCY ===', {
      mobileNo: regdMobileNo,
      newAC: newAssemblyFormData.ac_name
    });
    ...
    // Create new assembly object (clean data)
    const newAssembly = {
      ac_number: parseInt(newAssemblyFormData.ac_number),
      ac_name: newAssemblyFormData.ac_name.trim(),
      district: newAssemblyFormData.district.trim()
    };

    // Only add type if it's provided
    if (newAssemblyFormData.type && newAssemblyFormData.type.trim()) {
      newAssembly.type = newAssemblyFormData.type.trim();
    }

    // Combine existing + new (ensure clean data for existing ones too)
    ...
    // Prepare payload EXACTLY as in Postman
    const requestPayload = {
    ...
    ConstituencyLoggingService.constDebug('Add assembly request payload', {
    ...
    // IMPORTANT: Check if assemblyconstituencies already exists for this user
    ...
    if (checkResult.success && checkResult.data && 
      ...
      // No existing data - use POST to create
    ...
    if (!result.success) {
      ConstituencyLoggingService.constError('API request failed', {
    ...
    ConstituencyLoggingService.constInfo('✅ New assembly constituency added successfully');

    // Reset form and close modal
    setNewAssemblyFormData({
      ac_number: '',
      ac_name: '',
      district: '',
      type: ''
    ...
    Alert.alert('Success', 'New assembly constituency added successfully!');

  } catch (error) {
    ConstituencyLoggingService.constError('❌ Error adding assembly constituency', error);
    Alert.alert(
      'Add Failed', 
      `Failed to add assembly constituency:\n\n${error.message}\n\nPlease check:\n- Your internet connection\n- The server is running\n- You have proper permissions`
    );
  } finally {
...
const renderAssemblySegments = () => {
  ...
  if (error && assemblyConstituencies.length === 0) {
    return (
      <View style={styles.segmentsList}>
        ...
        {/* Add New Button - Show even on error */}
        {isAdmin && (
          <TouchableOpacity
  ...
  if (assemblyConstituencies.length === 0) {
    return (
      <View style={styles.segmentsList}>
        ...
        {/* Add New Button - Show when no data */}
        {isAdmin && (
          <TouchableOpacity
            style={styles.addAssemblyButton}
            onPress={() => setAddAssemblyModalVisible(true)}
  ...
  return (
    <View style={styles.segmentsList}>
      ...
      {/* Assembly List - WITHOUT individual edit buttons */}
  ...
  <Modal
    ...
    animationType="slide"
    transparent={true}
    onRequestClose={() => setAddAssemblyModalVisible(false)}
  >
    <View style={styles.modalOverlay}>
      <View style={styles.modalContent}>
        <View style={[styles.modalHeader, { backgroundColor: '#2980b9' }]}>
          <Icon name="add-circle" size={18} color="#fff" />
          <Text style={styles.modalTitle}>Add New Assembly Constituency</Text>
          <TouchableOpacity
            onPress={() => {
              setAddAssemblyModalVisible(false);
              setNewAssemblyFormData({
        ...
        <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
          <View style={styles.formGroup}>
            ...
            <TextInput
              style={styles.formInput}
              value={newAssemblyFormData.ac_number}
              onChangeText={(text) => setNewAssemblyFormData({ 
                ...newAssemblyFormData, 
                ac_number: text 
              })}
          ...
          <View style={styles.formGroup}>
            ...
            <TextInput
              style={styles.formInput}
              value={newAssemblyFormData.ac_name}
              onChangeText={(text) => setNewAssemblyFormData({ 
                ...newAssemblyFormData, 
                ac_name: text 
              })}
          ...
          <View style={styles.formGroup}>
            ...
            <TextInput
              style={styles.formInput}
              value={newAssemblyFormData.district}
              onChangeText={(text) => setNewAssemblyFormData({ 
                ...newAssemblyFormData, 
                district: text 
              })}
              placeholder="Enter district (e.g., Paschim Champaran)"
              placeholderTextColor="#bdc3c7"
            />
          </View>
          ...
          <View style={styles.formGroup}>
            ...
            <TextInput
              style={styles.formInput}
              value={newAssemblyFormData.type}
              onChangeText={(text) => setNewAssemblyFormData({ 
                ...newAssemblyFormData, 
                type: text 
              })}
        ...
        <View style={styles.modalFooter}>
          ...
          >
            {saveLoading ? (
              ...
              <Text style={styles.saveButtonText}>Save Assembly</Text>
            )}
          </TouchableOpacity>
        </View>
      </View>
